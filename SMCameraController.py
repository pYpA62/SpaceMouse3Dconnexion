...
class CameraController:
    ...
    def update_camera(self, filtered_values: Dict[str, float]) -> None:
        """
        Update camera position and rotation using filtered values
        ...
        try:
            # Move view (translation)
            self.navigation.moveView(
                -filtered_values['x'] * self._move_factor,
                -filtered_values['y'] * self._move_factor
            )
            ...
            if camera_pose := self.navigation.cameraPose():
                ...
                camera_pose.setHeadingAngle(
                    ...
                    filtered_values['yaw'] * self._rotation_factor
                )
                
                # Update pitch
                camera_pose.setPitchAngle(
                    camera_pose.pitchAngle() + 
                    filtered_values['pitch'] * self._rotation_factor
                )
            ...
            # Handle zoom
            if abs(filtered_values['z']) > self._z_threshold:
                zoom_value = filtered_values['z'] * self._zoom_factor
